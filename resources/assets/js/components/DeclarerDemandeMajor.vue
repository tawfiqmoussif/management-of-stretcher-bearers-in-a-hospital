...
<div class="row">
    ...
    <div class="col-md-6">
         ...
         <select v-model="id_service_source" type="text" name="hopital" class="browser-default custom-select" >
                    ...
                    <option v-for="service in servicesMaj" :key="service.id" :value="service.id"> {{service.intitule | upText  }}</option>
                </select>
        </div> 
          ...
          has_billet(){
           this.billet? this.billet=false:this.billet=true;
                ...
                loadService(){
                  ...
                  loadMajor(){
                    axios.get('api/serviceMajor').then(({data})=>(this.servicesMaj = data));  
                    console.log("rees",this.servicesMaj);
                    console.log("rees",this.id_service_source);
                  },
                loadHospitaliers(){
                  ...
                  sendDemande(){   
                    this.$Progress.start();
                     ...
                     if(this.id_service_source==this.id_service_destination){
                            this.$Progress.fail();
                             swal.fire("Échoué!", "Il y a quelque chose incorrecte.", "Attention");
                                      return false;
